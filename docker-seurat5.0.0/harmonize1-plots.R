...
outpdf <- paste(outdir, "/", case, ".pdf", sep="")
outtab <- paste(outdir, "/", case, ".tsv", sep="")
outRobj <- paste(outdir, "/", case, ".RData", sep="")

samples.md <- read.table(samples.md.fn, header=TRUE)
samples.md <- samples.md %>% filter(Case == case)
print(samples.md)
...
DimPlot(seurat, reduction = "umap") + labs(title=paste(case, "UMAP on PCA"))
DimPlot(seurat, reduction = "pca") + labs(title=paste(case, "PCA"))
DimPlot(seurat, reduction = "pca", dims=3:4)  + labs(title=paste(case, "PCA"))

seurat <- seurat %>% RunHarmony(group.by.vars = "orig.ident", plot_convergence = TRUE)

DimPlot(seurat, reduction = "harmony") + labs(title=paste(case, "Harmony"))
DimPlot(seurat, reduction = "harmony", dims=3:4)  + labs(title=paste(case, "Harmony"))

seurat <- seurat %>% RunUMAP(reduction = "harmony", dims=umap.dims)
DimPlot(seurat, reduction = "umap") + labs(title=paste(case, "UMAP on Harmony"))
dev.off()
